...
function cpabc_appointments_main_initialization()
{
    ...
    $params["HOME_VISITOR"] = sanitize_text_field($_POST["home_visitor"]);
    ...
    if(date("d", strtotime($pdateAndTime[0])) == date("d")){
       if(date("D") == "Sat"){
            echo '<script>alert("Please accept our sincere apologies but the Saturday service RSVP is now closed. Please sign up to attend the Sunday service.");
                window.history.back();   
            </script>';
            return;
       }elseif(date("D") == "Sun"){
            echo '<script>alert("Please accept our sincere apologies but the Sunday service RSVP is now closed. Please join us online for our streaming service.");
                window.history.back();
            </script>';
            return;
       }
    ...
    for ($n=0; $n<count($pdateAndTime); $n++)
    {
        ...
        $rows_affected = $wpdb->insert( CPABC_APPOINTMENTS_TABLE_NAME, array( 'calendar' => $selectedCalendar,
                                                                        ...
                                                                        'member_visitor' => "".sanitize_text_field(@$_POST["member_visitor"]),
                                                                        'question' => $buffer_A,
                                                                        'quantity' => (isset($_POST["abc_capacity"])?sanitize_text_field($_POST["abc_capacity"]):1),
                                                                        'buffered_date' => serialize($params)
                                                                         ) );
        if (!$rows_affected)
        {
            echo 'Insert error. Contact the administrator of the site.';
            exit;
        }
...
<?php  cpabc_process_ready_to_go_appointment($item_number, ''); ?>
<form action="<?php echo get_site_url(); ?>" name="ppform3" method="post">
</form>
...
function cpabc_process_cancel_go_appointment()
{
    ...
    if($_SERVER['REQUEST_METHOD'] == "POST" && isset($_POST['release_seats'])){
        $itemnumber = $_POST['seats_id'];
        $quantity = intval($_POST['release_seats']);

        if(is_numeric($itemnumber)){
            if($quantity == 0){

                $wpdb->query( "DELETE FROM wp_cpabc_appointments WHERE id=".$itemnumber );
                $wpdb->query( "DELETE FROM wp_cpabc_appointment_calendars_data WHERE id=".$itemnumber );
                header("Location: ".CPABC_APPOINTMENTS_DEFAULT_ON_CANCEL_REDIRECT_TO);
                exit;

            }else{

                $wpdb->query( "UPDATE wp_cpabc_appointments SET quantity = ".$quantity." WHERE id=".$itemnumber );
                $wpdb->query( "UPDATE wp_cpabc_appointment_calendars_data SET quantity = ".$quantity." WHERE id=".$itemnumber );
                header("Location: ".CPABC_APPOINTMENTS_DEFAULT_ON_CANCEL_REDIRECT_TO);
                exit; 
...
function cpabc_process_ready_to_go_appointment($itemnumber, $payer_email = "")
{
   ...
   for ($n=0;$n<count($itemnumber);$n++)
   {
       ...
       if ($n == count($itemnumber)-1) // send emails only once
       {
           ...
           $cancel_link = cpabc_appointment_get_FULL_site_url().'/index.php/rsvp-form/?cpabc_c=1&i='.base64_encode($itemnumberdb).'&a=1';
