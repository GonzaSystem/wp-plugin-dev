...
<?php if ($_GET["cpabc_c"] && $_GET["cpabc_c"] == 1): ?>
  
      <?php 

          $itemnumber = base64_decode($_GET["i"]);
            if (is_numeric($itemnumber)){
              $resultado = $wpdb->get_results( "SELECT quantity FROM wp_cpabc_appointments WHERE ID = ".$itemnumber );

              foreach ($resultado as $key => $value) {
                $total_seats = $value->quantity;
              }
           }
      ?>

      <form class="cpp_form" method="POST" name="ReleaseSeats" action="<?php get_site_url(); ?>">
          <label for="release_seats">Enter the new amount of people that will assist the event</label>
          <select type="text" class="form-control" name="release_seats" id="release_seats">
          ...
          <br>
          <br>
          <input type="hidden" name="seats_id" value="<?php echo $itemnumber ?>">
...
<input type="email" name="email" value="<?php /** if (isset($current_user->user_email)) echo $current_user->user_email; */ ?>" required><br />
...
<input type="hidden" id="total_household" name="total_household"> -->

<?php _e('Member-Visitor', 'appointment-booking-calendar'); ?>:<br />
<select type="text" name="member_visitor" required>
...
$(document).ready(function(){
  ...
  // Appending based on the # of people who will attend
    $('#household_people').on('change', function() {
      $("#extra_hh").remove();
      $("#extra_household").append("<div id='extra_hh'></div>");
        var totalPeople = document.getElementById('household_people').value;
      
      $("#total_household").val(totalPeople);
        var i;
  ...
  // Check if it's a child or dependent
  $(document).on('change', 'select#child_dependent', function(){
     var child_numb = $(this).attr('name');
     var child = child_numb.split('_');
     var parents_phone = $("#phone").val();
     ...
     // If it's not then we append the phone and email inputs to fill
        ...
        $("#child_dependent_data_"+child[2]).children().remove();
         $("#child_dependent_data_"+child[2]).append('<label for="child_dependent_phone_'+child[2]+'">Non Child/Dependent Phone</label> <br /><input type="text" name="child_dependent_phone_'+child[2]+'"> <br /><label for="child_dependent_email_'+child[2]+'">Non Child/Dependent Email</label><br /> <input type="text" name="child_dependent_email_'+child[2]+'"> <br /><br />')
     }
  })
});
</script> -->
